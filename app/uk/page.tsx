  ...
  function header(th: string, key: keyof Model) {
    return (
      <th
        ...
        onClick={() => {
          sortKey === key ? setAsc(!asc) : (setSortKey(key), setAsc(false));
        }}
      >
          ...
          {sorted.map((m, idx) => {
            ...
            return (
              <tr key={idx}>
                <td>
                  <button
                    className={`btn ${isSel ? "bg-gray-900 text-white border-gray-900" : ""}`}
                    onClick={() => onCompareToggle(m)}
                  >
                ...
                <td className="text-sm text-gray-600">
                  Mop: {m.mop_type} · Dock: {m.dock}
                  {m.suction_pa ? ` · Suction: ${m.suction_pa}Pa` : ""}
                </td>
        ...
        <table className="mt-3 min-w-[640px]">
          ...
          <tbody>
            <tr>
              ...
              {models.map((m, i) => (
                <td key={i}>{m.overall?.toFixed(2)}</td>
              ))}
            </tr>
            <tr>
              <td className="font-semibold">Spec</td>
              {models.map((m, i) => (
                <td key={i}>{m.spec?.toFixed(2)}</td>
              ))}
            </tr>
            <tr>
              <td className="font-semibold">Reviews</td>
              {models.map((m, i) => (
                <td key={i}>{m.review_score != null ? m.review_score.toFixed(2) : "—"}</td>
              ))}
            </tr>
            <tr>
              <td className="font-semibold">Value</td>
              {models.map((m, i) => (
                <td key={i}>{m.value?.toFixed(2)}</td>
              ))}
            </tr>
    ...
    <div className="grid gap-6">
      ...
      <div className="flex items-center gap-2">
        {Object.entries(bands).map(([k, v]) => (
          <button
            key={k}
            onClick={() => setBand(k as any)}
            className={`tab ${band === k ? "tab-active" : ""}`}
          >
            {v.label}
          </button>
        ))}
